...
def extract_data_from_hub_transcript(turns_of_talk: list):
    ...
    speaker_tags = [tag[0] if len(tag) > 0 else "" for tag in speaker_tags]
    speaker_tag_set = set(speaker_tags)

    for tag in speaker_tag_set:
        turns_of_talk = [turn.replace(tag + ":", "") for turn in turns_of_talk]
...
def word_to_transcript(doc_file=str):
    ...
    if "[0" in paragraphs[0]:
        return extract_data_from_hub_transcript(turns_of_talk=paragraphs)
